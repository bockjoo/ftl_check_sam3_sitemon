       ...
       for h in $(printf "$output\n" | grep -A 2 -B 3 $sf | grep "\"dst_hostname\"" | sort -u | cut -d\" -f4) ; do
          ...
          for legend in $(echo $lstring | sed "s#|# #g") ; do
             ...
             if [ "x$status" == "x" ] ; then
                ...
                echo "<td bgcolor='$bgcolor'><a href='${detail_link}' target=_blank> $status </a><a href='?test_id=$id&site=$thesite' target=_blank> $status </a><br/><FONT color='yellow'> T:$YmdHM <!-- $L --> </FONT> </td>"
             fi
