...
for thestring in $thestrings ; do
    echo "$thestring" | grep -q -e "test_howto\|test_id\|test_output\|site"
    [ $? -eq 0 ] || continue
    eval $thestring
    if [ "x$site" != "x" ] ; then
       thesite=$site
...
sed -e "s#@@thesite@@#$thesite#" -e "s#@@profile@@#$theprofile#" $inputs/${sitemon_agg_metric_json}.in > ${sitemon_agg_metric_json}_${now_is}
...
sed -e "s#@@thesite@@#$thesite#" -e "s#@@profile@@#$theprofile#" -e "s#@@size@@#$size#" -e "s#@@unique_f@@#$unique_f#" -e "s#@@tie_breakter_id@@#$tie_breaker_id#" $inputs/${sort_search_json}.in > ${sort_search_json}_${now_is}
...
for i in $(seq 0 $nsearch) ; do
   if [ $i -eq 0 ] ; then
      ...
      sed -e "s#@@thesite@@#$thesite#" -e "s#@@profile@@#$theprofile#" -e "s#@@size@@#$size#" -e "s|@@search_after@@|$search_after|" -e "s#@@unique_f@@#$unique_f#" -e "s#@@tie_breakter_id@@#$tie_breaker_id#" $inputs/${sort_search_search_json}.in > ${sort_search_after_json}_${now_is}
...
timestamps=$(sed 's#"data":#\n"data":#g' $(basename $0 | sed "s#\.sh##").*.out_${now_is} | sed 's#,"#,\n"#g' | grep "^\"_id\"\|^\"metric_name\"\|^\"dst_hostname\"\|^\"status\"\|^\"service_flavour\"\|^\"timestamp\"" | grep "\"timestamp\"" | cut -d: -f2 | cut -d\} -f1 | sort -rn | uniq)
LEGEND="
...
echo "<table>"
echo "<tr><td bgcolor='yellow'>Legend<td><td bgcolor='yellow' align='left'>Metric Name</td></tr>"
for l in $LEGEND ; do
   L=$(echo $l | cut -d: -f1)
   t=$(echo $l | cut -d: -f2)
   echo "<tr><td> $L </td><td align='left'> $t </td></tr>"
...
[ $nt -eq 10 ] || { echo "<pre>" Warning one or more CE has number of tests less than 10 "</pre>" : nt=$nt ; } ;
echo "<tr><td bgcolor='cyan'>Sitename<td><td bgcolor='cyan'>Flavor</td><td bgcolor='cyan'> Host </td>"
i=0
old_sf=
...
for sf in $(printf "$output\n" | grep "\"service_flavour\"" | sort -u | cut -d\" -f4) ; do
       ...
       for h in $(printf "$output\n" | grep -A 2 -B 3 $sf | grep "\"dst_hostname\"" | sort -u | cut -d\" -f4) ; do
          if [ "$h" == "$maxh" ] ; then
             [ $i -gt 1 ] && echo "<tr><td>&nbsp;<td><td>&nbsp;</td><td>&nbsp;</td>"
             for t in $(printf "$output\n" | grep -A 2 -B 3 $sf | grep -A 4 -B 1 $h | grep "\"metric_name\"" | cut -d\" -f4 | sort -u) ; do
                 ...
                 [ "x$L" == "x" ] || { echo "<td bgcolor='cyan'> <b> <a href='?test_howto=$m_name' target=_blank> $L </a> </b> </td>" ; lstring="${lstring}|$L" ; } ;
             done
             echo "</tr>"
         fi
       done
       ...
       for h in $(printf "$output\n" | grep -A 2 -B 3 $sf | grep "\"dst_hostname\"" | sort -u | cut -d\" -f4) ; do
          ...
          if [ $i -eq 1 ] ; then # First Service Flavor
             if [ $j -eq 1 ] ; then
                echo "<tr><td><a href='$profile_history_site_link' target=_blank> $thesite </a> <td><td><FONT color='green'>$sf</FONT></td><td> <a href='$profile_history_host_link' target=_blank> $h </a> </td>"
             else
                echo "<tr><td>&nbsp;<td><td>&nbsp;</td><td> <a href='$profile_history_host_link' target=_blank> $h </a> </td>"
             fi
          else
             if [ $j -eq 1 ] ; then
                echo "<tr><td>&nbsp;<td><td><FONT color='green'>$sf</FONT></td><td> <a href='$profile_history_host_link' target=_blank> $h </a> </td>"
             else
                echo "<tr><td>&nbsp;<td><td>&nbsp;</td><td> <a href='$profile_history_host_link' target=_blank> $h </a> </td>"
             fi
          fi
          ...
          for legend in $(echo $lstring | sed "s#|# #g") ; do
             ...
             #if [ "x$L" != "x" ] ; then
             if [ "x$status" == "x" ] ; then
                echo "<td bgcolor='$bgcolor'> <b><FONT color='yellow'> <a href='?test_howto=$L' target=_blank> $L </a> </FONT> </b> </td>"
             else
